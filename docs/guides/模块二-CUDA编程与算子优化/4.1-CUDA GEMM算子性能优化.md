...
description: "GEMM，本文从朴素实现出发，逐步Block-Warp-Thread三级Tiling优化、向量化访存、Bank Conflict 消除、双缓冲等优化手段，带你系统掌握 CUDA GEMM 优化的完整方法论。"
pubDate: 2026-06-02
category: "cuda-optimization"
order: 401
chapter: 4
tags: ["CUDA", "GEMM Kernel", "算子优化", "矩阵乘法"]
---
...
<img src="/AIInfraGuide/images/gemm-1.png" alt="GEMM 的定义" style="max-width: 40%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-2.png" alt="Thread Block 级 Tiling 优化" style="max-width: 40%; display: block; margin: 0 auto;" />
...
- **输出 tileC**（$128 \times 128$）：将tileC 划分为若干个 `C_BLOCK_Y` $\times$ `C_BLOCK_X` = `BLOCK_SIZE`($256$)的 `C_BLOCK` ，这里取 `C_BLOCK_X = C_BLOCK_Y = 16`，则X方向有 `Tn = BN / C_BLOCK_X` 个`C_BLOCK`， Y方向有 `Tm = BM / C_BLOCK_Y` 个`C_BLOCK`，每个线程需要在`Tm` $\times$ `Tn`个`C_BLOCK`的相同位置输出一个值，总共`Tm` $\times$ `Tn`个值
```
...
<img src="/AIInfraGuide/images/gemm-C_tile_bolcks.png" alt="每个线程负责填充每个C_BLOCK中相同的位置" style="max-width: 50%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-3.png" alt="Thread 级 Tiling 优化：寄存器级数据复用" style="max-width: 50%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-4.png" alt="Warp 级分块优化" style="max-width: 65%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-5.png" alt="float4 下 warp(4x8) 的 Transaction 分析" style="max-width: 80%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-7.png" alt="方案四：8 x 8 计算区域拆分为 4 x 4 子块" style="max-width: 80%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/gemm-8.png" alt="方案三：Z-order 线程映射" style="max-width: 80%; display: block; margin: 0 auto;" />
