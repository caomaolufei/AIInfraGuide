...
## 📑 目录
- [1. 先定义生产目标](#1-先定义生产目标)；[2. TP、DP 与 EP](#2-tpdp-与-ep)；[3. 量化](#3-量化)；[4. Speculative Decoding](#4-speculative-decoding)；[5. PD Disaggregation](#5-pd-disaggregation)；[6. Model Gateway 与 Router](#6-model-gateway-与-router)；[7. Benchmark 方法](#7-benchmark-方法)；[8. 与 vLLM 公平对比](#8-与-vllm-公平对比)；[9. 上线清单](#9-上线清单)；[10. 生产端到端检查清单](#10-生产端到端检查清单)；[11. Trade-off 与选型](#11-trade-off-与选型)；[总结](#总结)；[自我检验](#自我检验)；[参考资料](#参考资料)。
---
...
第 2–8 节已经解释并行、量化、Spec、PD、Router 与公平对比的机制。本节不再复述公式和参数，只保留把这些能力接入 Kubernetes、执行生产实验和注入故障时新增的步骤。

### 10.1 冻结实验与发布身份

- [ ] SGLang 固定为 0.5.17，模型与 Tokenizer 使用不可变 revision，镜像使用 digest。
- [ ] 使用 Router 时固定 `sglang-router==0.3.2`；使用 Mooncake 时把 `mooncake-transfer-engine` 的解析版本写入 lockfile。
- [ ] 保存完整启动参数、硬件/NVLink/NIC 拓扑、dataset hash、到达/长度/前缀复用分布与原始 JSONL。
- [ ] 基础公平实验与“各自最佳配置”分开；后者才允许启用 Radix、cache-aware Router、Spec、EP 或 PD 等框架特长。

### 10.2 单副本 Kubernetes 骨架

每个 Worker Pod 只运行一个推理副本，Pod 的 GPU limit 等于该副本所需 TP/EP GPU 数。以下确实是一个 `replicas: 1`、`TP=1` 的单副本教学骨架：
...
spec:
  replicas: 1
  selector: {matchLabels: {app: sglang-qwen}}
  template:
    metadata: {labels: {app: sglang-qwen}}
    spec:
      terminationGracePeriodSeconds: 120
      containers:
        - name: server
          image: registry.example/sglang:0.5.17@sha256:<digest>
          args: ["python", "-m", "sglang.launch_server",
                 "--model-path", "/models/qwen", "--host", "0.0.0.0",
...
探针路径必须在 v0.5.17 镜像中实测。Startup 覆盖权重加载与 Graph 捕获，Readiness 只在模型可服务后成功；不要因为短时 GPU 忙而用 Liveness 重启进程。扩到多副本后再加入 Gateway、反亲和、PDB 与 HPA，不要把这个单副本清单误当成高可用方案。

TP=8 通常要求一个 Pod 独占同节点 8 GPU，并显式约束 GPU SKU、NVLink/NVSwitch 域、CPU/NUMA、共享内存、NCCL/NIC 与模型卷本地性。除非平台专门支持且实验通过，不要让普通调度器把一个 TP 副本跨节点拆分。

### 10.3 PD 与数据面专项检查

Prefill、Decode 与 Gateway 分别使用 Headless Service/Service。每个 P/D Pod 注册 `role`、模型 revision、TP/DP layout、KV dtype/page size、transfer backend、NIC endpoint 与容量；Gateway 不得配对不兼容的 P/D，也不得在滚动升级时混用未经验证的 KV wire format。

Mooncake 上线前单独验证：

- [ ] 同构 TP 与异构 TP 的 gather/RDMA/scatter；
- [ ] 1/4/16 GiB KV、单流与并发流、NIC/NUMA 绑定；
- [ ] 注册、gather、network、scatter、同步的分段耗时与有效带宽；
- [ ] 超时、网络抖动、P/D 重启和客户端取消后的 staging buffer 回收；
- [ ] checksum/错误输出检测，以及失败请求与其他租户的隔离。

### 10.4 两阶段公平实验

**阶段 A：共同能力基线。** 固定模型/Tokenizer/模板、精度与 KV dtype、GPU 与 TP、context、安全显存余量、客户端机器和 workload；关闭非共同特性。使用 v0.5.17 的同一负载发生器，分别选择 `sglang-oai-chat` 与 `vllm-chat` backend。扫描低负载、SLO 拐点和过载点，每点至少 1000 请求且重复 $n\ge5$；保存实际 input/output token、finish reason、错误、TTFT/TPOT 和 goodput。

**阶段 B：场景能力。** 在阶段 A 后分别执行：

- 热前缀：固定共享前缀组与到达 trace，测 cold、warm、Worker 重启恢复、round-robin 与 cache-aware 路由；
- 结构化输出：固定同一 `response_format`、schema 与 prompt，测合法率、语义正确率、冷/热编译延迟和复杂 schema 错误；
- 各自最佳：固定 GPU 总数、质量门、workload、SLO 与故障冗余，只开放各框架已验证的优势配置。

输出提前 EOS 时按实际 token 报告，并保留长度分布。某框架不支持 schema keyword 或其他能力时，应报告功能不兼容，不能偷偷降级测试条件。最终以单位合格请求成本和 SLO goodput 决策；差异落在重复波动内时结论是“无稳定差异”。

### 10.5 Kubernetes 与故障注入

- **Worker crash**：记录 Router 摘除时间、剩余副本 P99 与 cache 冷迁移；已开始流式返回的请求不得无条件重放。
- **Gateway crash**：验证 Gateway 多副本、连接中断语义、Worker registry 与限流状态恢复。
- **GPU OOM**：Pod 退出 Ready，错误明确分类并降载，不反复重试同一超长请求。
- **Mooncake/NIC 故障**：验证 timeout、P/D cleanup、buffer 回收、降级和租户隔离。
- **Kubernetes drain**：先在 Router 摘流，再等待长流到 deadline；剩余容量必须满足 PDB/N-1 目标。
- **错误升级**：Canary 的质量、`response_format` 与 Tool 测试阻断发布；旧镜像、模型 revision、Router 与兼容配置可一起回滚。

实验完成的最低标准是：环境和配置可追溯，公平对比原始数据齐全，单副本与多副本边界明确，至少完成 Worker crash、GPU OOM、drain，以及启用 PD 时的数据面故障注入。
